...
isInitialObject C A = (B : Precategory.ob (UnivalentCategory.cat C)) →
                       isContr (Precategory.hom (UnivalentCategory.cat C) A B)

isInitialToIsUniqueArrow : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → (A : Precategory.ob (UnivalentCategory.cat C)) → isInitialObject C A → {B : Precategory.ob (UnivalentCategory.cat C)} → (f : Precategory.hom (UnivalentCategory.cat C) A B) → isUniqueArrow C f
...
InitialObjectIsUniqueHelp C A B AisInit BisInit = record {h = fst (AisInit B) ;
                                                          h⁻¹ = fst (BisInit A) ;
                                                          sec = isContr→isProp (BisInit B) (Precategory.seq (UnivalentCategory.cat C) (fst (BisInit A)) (fst (AisInit B)))
                                                          (Precategory.idn (UnivalentCategory.cat C) B) ;
                                                          ret = isContr→isProp (AisInit A) (Precategory.seq (UnivalentCategory.cat C) (fst (AisInit B)) (fst (BisInit A)))
