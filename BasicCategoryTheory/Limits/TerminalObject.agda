...
isTerminalObject C B = (A : Precategory.ob (UnivalentCategory.cat C)) → isContr (Precategory.hom (UnivalentCategory.cat C) A B)

isTerminalToIsUniqueArrow : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → (B : Precategory.ob (UnivalentCategory.cat C)) → isTerminalObject C B → {A : Precategory.ob (UnivalentCategory.cat C)} → (f : Precategory.hom (UnivalentCategory.cat C) A B) → isUniqueArrow C f
...
TerminalObjectIsUniqueHelp C A B AisTerm BisTerm = record {h = fst (BisTerm A) ;
                                                          h⁻¹ = fst (AisTerm B) ;
                                                          sec = isContr→isProp (BisTerm B) ((Precategory.seq (UnivalentCategory.cat C) (fst (AisTerm B)) (fst (BisTerm A))))
                                                          ((Precategory.idn (UnivalentCategory.cat C) B)) ;
                                                          ret = isContr→isProp (AisTerm A) ((Precategory.seq (UnivalentCategory.cat C) (fst (BisTerm A)) (fst (AisTerm B))))
                                                          ((Precategory.idn (UnivalentCategory.cat C) A)) }
