...
HelpSecEq : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : Precategory.ob (UnivalentCategory.cat C)} →
            (h : Precategory.hom (UnivalentCategory.cat C) A B) → (hinv : Precategory.hom (UnivalentCategory.cat C) B A) → Type ℓ'
HelpSecEq C {B = B} h hinv = Precategory.seq (UnivalentCategory.cat C) hinv h ≡ Precategory.idn (UnivalentCategory.cat C) B

HelpRetEq : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : Precategory.ob (UnivalentCategory.cat C)} →
            (h : Precategory.hom (UnivalentCategory.cat C) A B) → (hinv : Precategory.hom (UnivalentCategory.cat C) B A) → Type ℓ'
HelpRetEq C {A = A} h hinv = Precategory.seq (UnivalentCategory.cat C) h hinv ≡ Precategory.idn (UnivalentCategory.cat C) A
...
HelpSecEq2 : {ℓ ℓ' : Level} → (C : Precategory ℓ ℓ') → {A B : Precategory.ob C} → (h : Precategory.hom C A B) → (hinv : Precategory.hom C B A) → Type ℓ'
HelpSecEq2 C {B = B} h hinv = Precategory.seq C hinv h ≡ Precategory.idn C B

HelpRetEq2 : {ℓ ℓ' : Level} → (C : Precategory ℓ ℓ') → {A B : Precategory.ob C} → (h : Precategory.hom C A B) → (hinv : Precategory.hom C B A) → Type ℓ'
HelpRetEq2 C {A = A} h hinv = Precategory.seq C h hinv ≡ Precategory.idn C A
...
CatIsoCat≡ {C = C} isCat {A = A} {B = B} {x = x} {y = y} p q =
  CatIsoPreCat≡ p q
  (isProp→PathP (λ i → λ t s → homIsSet isCat (Precategory.seq C (q i) (p i)) (Precategory.idn C B) t s) (CatIso.sec x) (CatIso.sec y))
  (isProp→PathP (λ i → λ t s → homIsSet isCat (Precategory.seq C (p i) (q i)) (Precategory.idn C A) t s) (CatIso.ret x) (CatIso.ret y))
...
transportEq : {ℓ ℓ' : Level} → {A B : Type ℓ} → {F : Type ℓ → Type ℓ'} → A ≃ B → F A → F B
transportEq {F = F} A≃B FA = equivFun (cong≃ F A≃B) FA
