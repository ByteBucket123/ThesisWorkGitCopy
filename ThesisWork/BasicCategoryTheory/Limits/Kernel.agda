...
kernelFactors : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : ob (UnivalentCategory.cat C)} →
                (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → (ker : hom (UnivalentCategory.cat C) S A) →
                Type (ℓ-max ℓ  ℓ')
kernelFactors C {A = A} {S = S} hasZero f ker = {D : ob (UnivalentCategory.cat C)} (h : hom (UnivalentCategory.cat C) D A) →
...
KerFactorsEq C {A} {B} {S} hasZero f k p q uniFact = isProp→PathP (λ i → λ r s → implicitFunExt (λ {D} → funExt (λ h → funExt (λ hf=0 →
                                             ...
                                             (seq (UnivalentCategory.cat C) (uniFact i D (fst (r h hf=0)) (fst (s h hf=0))
...
hasKernel : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : ob (UnivalentCategory.cat C)} →
            hasZeroObject C → (f : hom (UnivalentCategory.cat C) A B) → Type (ℓ-suc (ℓ-max ℓ ℓ'))
hasKernel C hasZero f = ∥ ({S : ob (UnivalentCategory.cat C)} → Kernel C {S = S} hasZero f) ∥
--{S : ob (UnivalentCategory.cat C)} → Kernel C {S = S} hasZero f

hasKernelIsProp : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : ob (UnivalentCategory.cat C)} →
            (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → isProp (hasKernel C hasZero f)
...
hasAllKernels C hasZero = {A B : ob (UnivalentCategory.cat C)} → (f : hom (UnivalentCategory.cat C) A B) → hasKernel C hasZero f
