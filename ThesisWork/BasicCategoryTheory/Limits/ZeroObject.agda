...
record ZeroArrow {ℓ ℓ'} (C : UnivalentCategory ℓ ℓ') {Z : Precategory.ob (UnivalentCategory.cat C)} (A B : ob (UnivalentCategory.cat C)) : Type (ℓ-suc (ℓ-max ℓ ℓ')) where
  constructor ZeroArrowConst
  field
    f : hom (UnivalentCategory.cat C) A B
...
getZeroArrow C {A = A} {B = B} hasZero = record {f        = seq (UnivalentCategory.cat C) (fst (ZeroObject.isTerm (CategoryWithZeroObject.isZero hasZero) A)) (fst (ZeroObject.isInit (CategoryWithZeroObject.isZero hasZero) B)) ;
...
isZeroArrow : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : Precategory.ob (UnivalentCategory.cat C)} → hasZeroObject C → (f : hom (UnivalentCategory.cat C) A B) → Type ℓ'
isZeroArrow C {A = A} {B = B} hasZero f = {g : hom (UnivalentCategory.cat C) A (CategoryWithZeroObject.obj hasZero)} →
                                          {h : hom (UnivalentCategory.cat C) (CategoryWithZeroObject.obj hasZero) B} →
                                          seq (UnivalentCategory.cat C) g h ≡ f
...
ZeroArrowIsUnique C {A = A} {B = B} hasZero {h = h} {k = k} {j = j} {l = l} fZeroArrow gZeroArrow =
  (sym (fZeroArrow {h} {j})) ∙
  (cong₂ (λ g h → seq (UnivalentCategory.cat C) g h)
...
isZeroArrowIsProp : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : Precategory.ob (UnivalentCategory.cat C)} → (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → isProp (isZeroArrow C hasZero f)
isZeroArrowIsProp C {A = A} {B = B} isZero f p q = implicitFunExt λ {g} → implicitFunExt (λ {h} → homIsSet (UnivalentCategory.isCat C) (seq (UnivalentCategory.cat C) g h) f p q)
