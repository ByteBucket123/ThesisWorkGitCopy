...
coKernelFactors : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : ob (UnivalentCategory.cat C)} →
                (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → (ker : hom (UnivalentCategory.cat C) B S) →
                Type (ℓ-max ℓ  ℓ')
coKernelFactors C {B = B} {S = S} hasZero f ker = {D : ob (UnivalentCategory.cat C)} (h : hom (UnivalentCategory.cat C) B D) →
                 seq (UnivalentCategory.cat C) f h ≡ (ZeroArrow.f (getZeroArrow C hasZero)) →
                 Σ (hom (UnivalentCategory.cat C) S D)
                 (λ h' → (seq (UnivalentCategory.cat C) ker h' ≡ h))
                   
record CoKernel {ℓ ℓ'} (C : UnivalentCategory ℓ ℓ') {A B S : ob (UnivalentCategory.cat C)}
       (hasZero : CategoryWithZeroObject C) (f : hom (UnivalentCategory.cat C) A B) : Type (ℓ-suc (ℓ-max ℓ  ℓ')) where
  constructor kernelConst
  field
    coKer : hom (UnivalentCategory.cat C) B S
    coKerComp : seq (UnivalentCategory.cat C) f coKer ≡ (ZeroArrow.f (getZeroArrow C hasZero))
    coKerFactors : coKernelFactors C hasZero f coKer
    coKerFactorsUnique : isEpic C coKer


isCoKernel :  {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : ob (UnivalentCategory.cat C)} →
              hasZeroObject C → (f : hom (UnivalentCategory.cat C) A B) → (k : hom (UnivalentCategory.cat C) B S)
...
HelpCoKerComp : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : Precategory.ob (UnivalentCategory.cat C)}
                 ...
                 (coKer : hom (UnivalentCategory.cat C) B S) → Type ℓ'
HelpCoKerComp C hasZero f coKer = seq (UnivalentCategory.cat C) f coKer ≡ (ZeroArrow.f (getZeroArrow C hasZero))

HelpCoKerFactorsUnique : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {B S : Precategory.ob (UnivalentCategory.cat C)}
                         (ker : hom (UnivalentCategory.cat C) B S) → Type (ℓ-max ℓ ℓ')
...
CoKernel≡ : {ℓ ℓ' : Level} → {C : UnivalentCategory ℓ ℓ'} → {A B S : Precategory.ob (UnivalentCategory.cat C)}
                 → {hasZero : CategoryWithZeroObject C} {f : hom (UnivalentCategory.cat C) A B} {x y : CoKernel C {S = S} hasZero f}
...
CoKerEq : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : ob (UnivalentCategory.cat C)} →
        (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → (k : hom (UnivalentCategory.cat C) B S)
         → (p q : isCoKernel C hasZero f k) → CoKernel.coKer (fst p) ≡ CoKernel.coKer (fst q)
CoKerEq C hasZero f k p q = (snd p) ∙ (sym (snd q))

CoKerCompEq : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : ob (UnivalentCategory.cat C)} →
            (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → (k : hom (UnivalentCategory.cat C) B S)
...
CoKerCompEq C hasZero f k p q = (isProp→PathP (λ i → homIsSet ((UnivalentCategory.isCat C)) (seq (UnivalentCategory.cat C) f ((CoKerEq C hasZero f k p q) i)) ((ZeroArrow.f (getZeroArrow C hasZero)))) (CoKernel.coKerComp (fst p)) (CoKernel.coKerComp (fst q)))

CoKerFactorsEq : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B S : ob (UnivalentCategory.cat C)} →
                 (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → (k : hom (UnivalentCategory.cat C) B S)
...
hasCoKernel : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : ob (UnivalentCategory.cat C)} →
            hasZeroObject C → (f : hom (UnivalentCategory.cat C) A B) → Type (ℓ-suc (ℓ-max ℓ ℓ'))
hasCoKernel C hasZero f = ∥ ({S : ob (UnivalentCategory.cat C)} → CoKernel C {S = S} hasZero f) ∥
--{S : ob (UnivalentCategory.cat C)} → Kernel C {S = S} hasZero f

hasCoKernelIsProp : {ℓ ℓ' : Level} → (C : UnivalentCategory ℓ ℓ') → {A B : ob (UnivalentCategory.cat C)} →
                  (hasZero : hasZeroObject C) → (f : hom (UnivalentCategory.cat C) A B) → isProp (hasCoKernel C hasZero f)
...
hasAllCoKernels C hasZero = {A B : ob (UnivalentCategory.cat C)} → (f : hom (UnivalentCategory.cat C) A B) → hasCoKernel C hasZero f
