...
ModuleHomoΣShort :  {ℓ : Level} → {R : CommutativeRing {ℓ}} → (M N : Module R) → Type ℓ
ModuleHomoΣShort M N =
  Σ (⟨ M ⟩M → ⟨ N ⟩M) (λ h → Σ (helpModuleHomomorphismLinear {M = M} {N = N} h)
                               (λ x → helpModuleHomomorphismScalar {M = M} {N = N} h))

ModuleHomoΣ→ModuleHomo : {ℓ : Level} → {R : CommutativeRing {ℓ}} → {M N : Module R} → ModuleHomoΣShort M N → ModuleHomomorphism R M N
ModuleHomoΣ→ModuleHomo (h , lin , sca) = moduleHomo h lin sca

ModuleHomo→ModuleHomoΣ : {ℓ : Level} → {R : CommutativeRing {ℓ}} → {M N : Module R} → ModuleHomomorphism R M N → ModuleHomoΣShort M N
ModuleHomo→ModuleHomoΣ (moduleHomo h linear scalar) = h , (linear , scalar)

ModuleHomoΣIso : {ℓ : Level} → {R : CommutativeRing {ℓ}} → {M N : Module R} → Iso (ModuleHomomorphism R M N) (ModuleHomoΣShort M N)
ModuleHomoΣIso =
  record {fun = ModuleHomo→ModuleHomoΣ ;
          inv = ModuleHomoΣ→ModuleHomo ;
          rightInv = λ z → refl ;
          leftInv = λ z → refl}

ModuleHomoΣEquiv : {ℓ : Level} → {R : CommutativeRing {ℓ}} → {M N : Module R} → ModuleHomomorphism R M N ≃ ModuleHomoΣShort M N
ModuleHomoΣEquiv = isoToEquiv (ModuleHomoΣIso)

isSetModuleHomoΣ : {ℓ : Level} → {R : CommutativeRing {ℓ}} → (M N : Module R) → isSet (ModuleHomoΣShort M N)
isSetModuleHomoΣ M N = isSetΣ (λ h k → isPropModuleFunc {M = M} {N = N} {h = h} {k = k}) λ h →
                       isProp→isSet (ModuleHomoPropΣIsProp {M = M} {N = N} h)

isSetModuleHomo : {ℓ : Level} → {R : CommutativeRing {ℓ}} → (M N : Module R) → isSet (ModuleHomomorphism R M N)
isSetModuleHomo M N = isSetEq (isSetModuleHomoΣ M N) (invEquiv ModuleHomoΣEquiv)
